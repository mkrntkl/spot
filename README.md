...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create spot
  fly apps create spot-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app spot
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app spot-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'spot-staging' does not match app name in config file 'spot'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name spot-db
  fly postgres attach --app spot spot-db

  fly postgres create --name spot-staging-db
  fly postgres attach --app spot-staging spot-staging-db
  ```
